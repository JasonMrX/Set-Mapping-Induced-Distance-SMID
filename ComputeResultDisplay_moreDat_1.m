...
% 4: pmse, Nmatch1, VScore1,  Nmatch2, VScore2
  ...
  [SimID, NotsimID, SimMetric, NotsimMetric, SimName, NotSimName] = ReadMetric('ComputeResult_moreDat_P2x2_MVE_LB8x8_EMD_HI_LB8x8Org_LBMVE.dat', 7) ;
...
%   plot(rocCurve(1, :), rocCurve(2, :), 'k', 'LineWidth', 2); 
  ...
  rocCurve = ComputeROC(SimMetric(:, 6), NotsimMetric(:, 6));
  plot(rocCurve(1, :), rocCurve(2, :), 'b', 'LineWidth', 2); 
  
  rocCurve = ComputeROC(SimMetric(:, 7), NotsimMetric(:, 7));
  plot(rocCurve(1, :), rocCurve(2, :), 'k', 'LineWidth', 2); 
% 
%   rocCurve = ComputeROC(SimMetric(:, 5), NotsimMetric(:, 5), 2);
%   plot(rocCurve(1, :), rocCurve(2, :), 'c', 'LineWidth', 2); 
%   legend('p2x2', 'MVE', 'LB8x8', 'EMD', 'HI');
  legend('LB8x8', 'MVE', 'LB8x8Org', 'LBMVE');
...
%       xlim([0 2000]);
      ...
      figure,
      SimDist = SimMetric(SelectSet1, 6);
      SimDistCost = SimMetric(SelectSet1, 7);
      NotSimDist = NotsimMetric(SelectSet2, 6);
      NotSimDistCost = NotsimMetric(SelectSet2, 7);
      plot(SimDist, SimDistCost, 'g.');
      hold on;
      plot(NotSimDist, NotSimDistCost, 'r.');
      title('LB8x8Org - LBMVE');
