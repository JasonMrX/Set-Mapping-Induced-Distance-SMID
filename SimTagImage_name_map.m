function SimTagImage_name_map(SSL_ref_in_Notview)
% modify SSLref_4_tagsystem(SSL_ref_in_Notview)  in C:\xiangyu\perforce\Categorize\AllPictures\TestSet
%to generate a mapping from
% a pair-image name to two image names
% Nov. 1, 2013

     pairnamefile = 'Int_pairname.dat';
     
     foutimage = fopen(pairnamefile, 'w');
...
% Changed based on ProgressiveReview_SSLref(SSL_ref_in_Notview, SSL_ref_outfile, clusterfile)
   ...
   if 1
       ...
       name2 = SSL_in(n).name2;
           ...
           fprintf(foutimage,  '%s  %s   %s\n', newfilename, name1, name2);
