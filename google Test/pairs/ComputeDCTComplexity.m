function [mDistPixel, mDistEntropy, mDistDCT, mDistDCTEntropy, success] = ComputeDCTComplexity(filename1, filename2)
...
%    [blkcost_layer2, ~, motionVectors] = DoEmdondMatchLayer2(Cedges, X1L-mean(mean(X1L)), X2L-mean(mean(X2L)));
%    
...
%    ===========================================
   % in DCT
    ...
    LargeC1 = zeros((rowL-8)*64, (colL-8));
    LargeC2 = zeros((rowL-8)*64, (colL-8));

    pos = 1 ;
    for m=1:8
        for n=1:8
            X1 = X1oriL(m:rowL-9+m, n:colL-9+n);     
            X2 = X2oriL(m:rowL-9+m, n:colL-9+n);     
            %X1 = X1ori(5:row-9+5, 5:col-9+5);     
            %X2 = X2ori(5:row-9+5, 3:col-9+3);     
            ...
            LargeC1( (pos-1)*(rowL-8)+1 : (pos)*(rowL-8), 1:colL-8) = C1 ;
            LargeC2( (pos-1)*(rowL-8)+1 : (pos)*(rowL-8), 1:colL-8) = C2 ;
...
%     ================= mv entropy ====================
    ...
    DCTImage1 = LargeC1(1 : 8 : rowL - 8, 1 : 8 : end);
    DCTImage2 = LargeC2(1 : 8 : rowL - 8, 1 : 8 : end);
    [~, Cedges, blkcost_layer1] = DoEdmondMatch(DCTImage1 - mean(DCTImage1(:)), DCTImage2 - mean(DCTImage2(:)), 1) ;
    mv = [mv; getMVFromEdges(Cedges, size(DCTImage1, 2))];
    
    DCTImage1 = LargeC1(1 : 8 : rowL - 8, 2 : 8 : end);
    DCTImage2 = LargeC2(1 : 8 : rowL - 8, 2 : 8 : end);
    [~, Cedges, blkcost_layer1] = DoEdmondMatch(DCTImage1 - mean(DCTImage1(:)), DCTImage2 - mean(DCTImage2(:)), 1) ;
    mv = [mv; getMVFromEdges(Cedges, size(DCTImage1, 2))];
    
    DCTImage1 = LargeC1(2 : 8 : rowL - 8, 1 : 8 : end);
    DCTImage2 = LargeC2(2 : 8 : rowL - 8, 1 : 8 : end);
    [~, Cedges, blkcost_layer1] = DoEdmondMatch(DCTImage1 - mean(DCTImage1(:)), DCTImage2 - mean(DCTImage2(:)), 1) ;
    mv = [mv; getMVFromEdges(Cedges, size(DCTImage1, 2))];
...
%     ================= mv entropy ====================
    ...
    mDistDCT = ComputeEMD4TwoDCTImage(LargeC1, LargeC2)/64 ;
