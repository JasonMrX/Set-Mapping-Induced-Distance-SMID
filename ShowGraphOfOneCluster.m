function ShowGraphOfOneCluster(SimScoreFilename, onefilename)
% Read SimScoreFilename and ClusterFilename as SSL_to_cluster does, and
% generate a LInkMatrix and a SimSets, based on which we read onefilename
% and show the graphy structure of the cluster that contains that file.
%
%
% Modify from SSL_to_cluster(SimScoreFilename, ClusterFilename)
% SSLfile for input,   ClusterFile as output

% SSLfile can be generated by the cluster.exe (with #GeneratePairSimilarityDecision set to 1)

%clusterFile is to be examined by DisplayFilesInClustersWithIDs.m


  SSL = ReadSimScoreList(SimScoreFilename) ;
  ...
  LinkMatrix = ScoreMatrix>=0.8 ;
  
  SimSets = FromMatrixToSets( LinkMatrix ) ;
  
  % Search the SimSets to identify which cluster this file falls into
  ClusterID = IdentifyOneCluster( SimSets, FileNameList, onefilename)  ;
  
  showgraph(SimSets(ClusterID).set, FileNameList, LinkMatrix);
  
function showgraph(cluster, FileNameList, LinkMatrix)
  clustersize = length(cluster);
  for s1=1:clustersize
      for s2 = s1+1:clustersize
          if( LinkMatrix(cluster(s1), cluster(s2)==1) )
            Show2image( cluster(s1), cluster(s2), FileNameList);
          end
      end          
  end

function Show2image(ID1, ID2, namelist)
        
         filename1 = namelist(ID1).name ;
         filename2 = namelist(ID2).name ;
         
         figure
         subplot(1,2,1), imshow(filename1);
         subplot(1,2,2), imshow(filename2);
         
         s = sprintf('%d %d', ID1,ID2);
         title(strcat(filename1,'-', filename2, '-', s));
         
function ClusterID = IdentifyOneCluster( SimSets, FileNameList, onefilename)  
  ...
  for n= 1:NumSets
      ...
      for s=1:SetSize
         if(strcmp(FileNameList( thisset(s) ).name, onefilename)==1)
             ClusterID = n ; return ; 
         end
      end
  end
